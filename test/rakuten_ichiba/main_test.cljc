(ns rakuten_ichiba.main-test
  "Contract + behavioral test for the rakuten_ichiba-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "rakuten_ichiba-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'rakuten_ichiba.main-test)]
