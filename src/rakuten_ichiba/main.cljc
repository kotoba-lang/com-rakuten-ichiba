(ns rakuten_ichiba.main
  "Kotodama WASM entrypoint for the rakuten_ichiba clean-room actor (L4) — Clojure port.
  ...
  facts (`rakuten_ichiba.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "rakuten_ichiba")
(def tier "L4")
...
(def entity-specs
  [{:entity "Product"   :plural "products"   :id-prefix "rakuteni_pro"
   ...
   {:entity "Customer"  :plural "customers" :id-prefix "rakuteni_cus"
    :fields [:email :firstName :lastName :phone] :required [:email :firstName]
    :coerce {} :refs {}}
   {:entity "Collection" :plural "collections" :id-prefix "rakuteni_col"
...
(defn emit-facts
  "EAVT facts for one record: {\"rakuten_ichiba.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "rakuten_ichiba-compat" :tier tier :entities entities} 200])
...
(defn start! [] :rakuten_ichiba-compat/ready)
